...
本目录按 `phases/` 的执行流程组织真实可执行的三语言模板。模板是项目脚本的源材料：实际执行时，把相关脚本复制或改写到 `paper-workspace/04-analysis/scripts/`，再用 `python3`、`Rscript` 运行；Stata 统一经 statamcp 的 `stata_run_file` 执行。
