...
| Agent | 文件路径 | 产出重点 |
|---|---|---|
| variable-inventory | `agents/variable-inventory-consultant.md` | 候选变量、来源路径、字段含义风险 |
| variable-role-mapping | `agents/variable-role-mapping-consultant.md` | Y/X/control/fe/cluster/weight/id/time/mechanism/mediator/moderator/heterogeneity/threshold/nonlinear/instrument/treatment/post/running/cutoff 映射和回流项 |
| variable-quality | `agents/variable-quality-consultant.md` | 缺失、类型、异常值、重复、特殊缺失码、面板唯一性和样本口径风险 |
...
`agents/variable-cleaning-consultant.md`
...
- Stata：参考 `templates/stata-analysis-template.do` 的读取、样本标记和描述统计段。
- R：参考 `templates/r-analysis-template.R` 的 `load_data()`、`clean_data()`、`make_table1()`。
- Python：参考 `templates/python-analysis-template.py` 的 `load_data()`、`clean_data()`、`describe_data()`。
